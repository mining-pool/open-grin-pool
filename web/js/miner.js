  ...
  if (status === 200) {
    ...
    for (const agent in miner.agents) {
      ...
      let textnode = document.createTextNode(
        JSON.stringify(miner.agents[agent])
      );
      average_hashrate =
        average_hashrate + miner.agents[agent].average_hashrate;
      realtime_hashrate =
        realtime_hashrate + miner.agents[agent].realtime_hashrate;
      ...
      document.getElementById("totalAHS").innerText =
        average_hashrate / 1000 + " kh/s";
      document.getElementById("totalRHS").innerText =
        realtime_hashrate / 1000 + " kh/s";
